...
class WallpaperPicker:
    def __init__(self, app: Gtk.Application):
        ...
        self.repository = WallpaperRepository(
            self.wallpaper_dir
        )
    ...
    def apply_color_filter(self, color_group):
        ...
        self.wallpapers = (
            self.repository.filter_by_color(
                color_group
            )
        )
    ...
    def apply_search_filter(self, query):
        if query:
            self.wallpapers = (
                self.repository.filter_by_name(
                    query
                )
            )
        else:
    ...
    def start_cache_generation(self):
        ...
        try:
            ...
            print(
                f"Failed to run cache script: {exc}",
                file=sys.stderr
            )
    ...
    def animate(
        ...
        scroll_done = (
            abs(scroll_delta) < 0.5
        )

        selection_done = (
            abs(selection_delta) < 0.01
        )
    ...
    def point_to_index(
        ...
        if (
            y < self.panel_y
            or y > self.panel_y + self.panel_h
        ):
            return None
    ...
    def quit(self):
        if self.animation_source:
            self.area.remove_tick_callback(
                self.animation_source
            )
